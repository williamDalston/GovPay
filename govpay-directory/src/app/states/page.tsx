      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
          {US_STATES.map((state, index) => (
            <AnimateOnScroll key={state.slug} delay={Math.min(index, 15) * 20}>
              <Link
              ...
              >
                <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-navy-800 font-data text-xs font-bold text-accent-blue group-hover:bg-accent-blue group-hover:text-white sm:h-10 sm:w-10 sm:text-sm">
                  {state.abbreviation}
                </span>
